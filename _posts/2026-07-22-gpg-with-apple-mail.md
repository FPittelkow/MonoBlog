...
1. **Install everything** – AppleMail (obviously), GPG Suite (we only use the Keychain, managing GPG with the termial sucks), MailGPG via Homebrew (`brew install --cask mailgpg` or whatever the brew formula is called).  
2. **Allow & activate** the MailGPG plugin
3. **Do NOT use** the “Mail‑Plugin” that ships with GPG Suite.
4. **Key management**: generate your key (or import an existing one) using the GPG Keychain that ships with GPG Suite. Upload the public key somewhere, because strangers on the internet want to send unsolicited encrypted blobs.  
